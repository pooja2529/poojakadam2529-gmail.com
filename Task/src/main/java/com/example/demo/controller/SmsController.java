...
import com.example.demo.service.ValidateDynamicField;
...
public class SmsController {
	...
	public ResponseEntity<?> verifyNotification(@RequestBody Sms track ) 
	{
		...
		String message=track.getTemplatemsg();
		Map<Integer,SmsMaster> result=smsMasterService.getAvailableNotification(track.getTokenid());
		
		if ( mob.equalsIgnoreCase(result.get(id).getMob()) && message.equalsIgnoreCase(result.get(id).getSms_template()))
		{
